...
def extract(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if opt.distill:
        ...
        # torch.save(dataset, "distill_dataset.pth")
        dataset.save("./zeroqyolodata")
        input, Y = next(iter(dataset))
        inputs = input.unsqueeze(0)
        kqat.disable_all_quant(model)
        model.to('cpu')
        torch.onnx.export(model,               # model being run
                            inputs,                         # model input (or a tuple for multiple inputs)
                            "./zeroqyolodata/yolov5.onnx",   # where to save the model (can be a file or file-like object)
                            export_params=True,        # store the trained parameter weights inside the model file
                            opset_version=11,          # the ONNX version to export the model to
...
if __name__ == '__main__':
    ...
    extract(hyp, opt, device, tb_writer, wandb)
